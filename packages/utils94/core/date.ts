...
import { CreateMonthArguments, Day, DayData } from '../interface/date.interface';
 ...
 */
export function createMonth({ date, data, weekStart, isSliceByWeek }: CreateMonthArguments = {}): Day[][] | Day[] {
    ...
    if (i === 1) {
      if (weekStart !== week) {
        ...
        for (let _i = mendDays; _i > 0; _i--) {
          ...
          const _dateText = `${dateInfo.year}/${fillZero(dateInfo.month)}/${fillZero(dateInfo.day)}`;
          res.push({
            source: prevDate,
            text: _dateText,
            current: false,
 ...
 * @returns boolean
 * @description
 * 基本的思路，日期也可以当成字符串进行比较，把开始日期，结束日期分别存进两个数组，并用sort排序，循环遍历数组，
 * 从开始时间的第二个元素去比较结束时间的第一个元素，
 * 如果小于，就代表时间段有交叉，直接跳出，不然就继续遍历，遍历结束，说明时间没有重复，可以提交。
 */
